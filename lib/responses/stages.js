...
module.exports.register = function()
{
    ...
    this.actions["!stage"] = function(message)
    {
        ...
        for (var cID of stages[user][stageName].destinations)
        {
            ...
            {
                results.push("- Would not move issue #" + issueNumber + " to column " + self.cachedProjects[pID].columns[cID].name + " - already there.");
            }
            else
            {
                results.push("- Would move issue #" + issueNumber + " to column " + self.cachedProjects[pID].columns[cID].name + " - card ID " + cardID + ".");
            }
        }

        for (var username of stages[user][stageName].assignees)
        {
            results.push("- Would reassign to " + username);
        }
